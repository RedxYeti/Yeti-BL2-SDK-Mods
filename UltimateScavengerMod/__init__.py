...
from ..ModMenu import EnabledSaveType, Options, RegisterMod, SDKMod
from typing import Any, Iterator, List, Optional
from .Rarity_Colors import GetRarityColorHex
from .HUDMessagesQueue import MessageQueue
import time
import math
...
#Action Skill Stuff
#Krieg just needs weapons no to be equipped
#Gunzerker is another story
def Kreiging(PC) -> bool:
...
#this just stops the weapon swap after removing a readied weapon
def SwitchWeapon(caller: UObject, function: UFunction, params: FStruct):
    return False
...
def AddItem(NewItem, PC, bPickupable, Classname, PlayerInventory, ItemToReplace) -> str:
    if Classname == "WillowWeapon":
        ...
        if LastWeaponWasEquipped and not Kreiging(PC):
...
#this safely removes all weapons from the weaponSlots
